...
- =:stdin= :: - A single =JSON= file using clingos output option =--outf=2=.
...
- =format= :: - Format to save the graph ={pdf|png|svg}=, defaults to =pdf=
  - if =:file= header argument is used, then =:format= is set to the filename extension
