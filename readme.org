...
- If you use ~use-package~, you can also write
  ...
  #+end_src
** Dependencies
In order to use it you must have [[https://clingraph.readthedocs.io/en/latest/][Potassco clingraph]] and [[https://graphviz.org/download/][graphviz]] installed.
...
- =:out= :: - Type of output ={facts|dot|render|animate|tex}=, defaults to =facts=
  - if =:file= header argument is used, then =:out= is set to =animate= if file name extension is =gif=
    else it is set to =render=
...
- =:engine= :: Layout command used by [[https://graphviz.org/][graphviz]] {dot|neato|twopi|circo|fdp|osage|patchwork|sfdp},
  defaults to =dot=
